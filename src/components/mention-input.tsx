  ...
  return (
    <div className="relative">
      ...
      {!showCommandSuggestions && showSuggestions && suggestions.length > 0 && (
        <div className="absolute bottom-full left-0 mb-2 w-64 bg-black/90 border border-white/10 rounded-xl shadow-2xl shadow-purple-500/10 z-50 max-h-48 overflow-y-auto backdrop-blur-xl scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
          <div className="p-2 text-xs font-bold text-white/50 uppercase tracking-wider sticky top-0 bg-black/90 backdrop-blur-xl border-b border-white/5">
            Suggestions
          </div>
          <div className="p-1">
            {suggestions.map((suggestion, index) => (
              <div
                ...
                className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-all duration-200 ${
                  index === selectedSuggestionIndex ? 'bg-white/10 text-white' : 'text-white/70 hover:bg-white/5 hover:text-white'
                }`}
              ...
              >
                ...
                {suggestion.collaborator.role && (
                  <div className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-white/40 capitalize border border-white/5">
