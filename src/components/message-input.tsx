...
export function MessageInput({ onSendMessage, disabled, repoFullName }: MessageInputProps) {
  ...
  return (
    <div className="relative group">
      <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-500/20 to-cyan-500/20 rounded-xl blur opacity-0 group-hover:opacity-100 transition duration-500"></div>
      <div className="relative bg-black/40 rounded-xl backdrop-blur-xl border border-white/10">
      {isInCommandMode ? (
        ...
        >
          <Textarea
            ...
            className="pr-20 min-h-[50px] bg-transparent border-0 focus-visible:ring-0 resize-none py-3 px-4 text-white placeholder:text-white/30"
            rows={1}
            disabled={disabled}
          />
        </CommandPopover>
      ...
      <Button
        type="submit"
        size="icon"
        className="absolute bottom-2 right-2 h-8 w-10 rounded-lg bg-white/10 hover:bg-white/20 text-white transition-all duration-200 disabled:opacity-50"
        onClick={handleSend}
