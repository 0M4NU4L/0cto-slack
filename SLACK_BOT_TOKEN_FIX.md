...
- `conversations:history` - Read conversation messages (CRITICAL for /0cto analyze)
...
**For each channel where you want to use `/0cto`:**
```
/invite @0cto
```
...
- **Fix:** Bot needs to be added to the channel: `/invite @0cto`
...
- [ ] Bot added to test channels with `/invite @0cto`
- [ ] `/0cto status` returns all ✅
- [ ] `/0cto analyze` works without auth errors
