# 0cto Slack Bot - Troubleshooting dispatch_failed Error

## Problem
The `/0cto` command is failing with "dispatch_failed" error in Slack.
...
**Debugging Steps:**
1. Use `/0cto status` to check configuration
...
If issues persist, the `/0cto status` command will show exactly which configuration is missing.